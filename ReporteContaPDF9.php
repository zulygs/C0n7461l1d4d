...
header('Content-Disposition: attachment; filename=SerieOF/' . $Fecha . ' al ' . $Fechaa . '.xls');
...
$Consulta = "SELECT a.dia_Fecha,dbo.alote(a.inm_id) as lote, dbo.recuperaNombreInmueble(a.inm_id) as nombre, 
    a.tra_Codigo,a.tra_Descripcion,b.ser_corr,a.debe,a.haber,a.corrusr,a.rec_id
    from ecuenta a
    left join resolucionAgPrincipal b on
    a.inm_id=b.inm_id and a.rec_id=b.rec_id
    where
    cast(a.dia_Fecha as date) between '$Fecha' and '$Fechaa'  and a.codAgencia=1
    and a.tra_Codigo in(select tra_Codigo from agu_TransaccionesCaja where genfesn='S') and
    a.USUARIO <>'Sistemas'
...
echo "<table border='1' cellpadding='2' cellspacing='0' width='100%'>
   <caption><font color='red' size='10'>Serie OF</font></caption>
    <tr>
        <td>Fecha</td>
        <td>Lote</td>
        <td>Nombre</td>
...
while ($Fila = sqlsrv_fetch_array($R)) {
    ...
    <tr>
        ...
        <td ><?php echo $rec_id_; ?></td>
        <td ><?php echo $corrusr_; ?></td>
    </tr>
