...
header('Content-Disposition: attachment; filename=SerieD/'.$Fecha.' al '.$Fechaa.'.xls');
...
from ecuenta where cast(dia_Fecha as date) between '$Fecha' and '$Fechaa'   and
tra_Codigo in(select tra_Codigo from agu_TransaccionesCaja where   tra_id IN(47,40) AND autorizacion ='serie')  order by dte";
...
a.tra_Descripcion,b.ser_corr,a.debe,a.haber
        ...
        and  a.tra_Codigo not in('PAT','CA','LC','NC') AND a.codAgencia=3 and a.efecto=1 
    ORDER BY a.dia_Fecha, a.mov_NoDoctoSELECT dia_Fecha,dbo.alote(inm_id) as lote, dbo.recuperaNombreInmueble(inm_id) as nombre, tra_Codigo,tra_Descripcion,mov_NoDocto,debe,haber
                from ecuenta where cast(dia_Fecha as date) between '$Fecha' and '$Fechaa'
        and mov_NoDocto not in(select ser_corr from resolucionAgMega )  
        and  tra_Codigo not in('PAT','CA','LC','NC') AND codAgencia=3 and efecto=1 ORDER BY dia_Fecha, mov_NoDocto"
        SELECT dia_Fecha,dbo.alote(inm_id) as lote, dbo.recuperaNombreInmueble(inm_id) as nombre, tra_Codigo,tra_Descripcion,dte,debe,haber
                from ecuenta where cast(dia_Fecha as date) between '$Fecha' and '$Fechaa'  and
                tra_Codigo in(select tra_Codigo from agu_TransaccionesCaja where  ser_id=7)  order by dte";*/
                $R=sqlsrv_query($con,$Consulta);

                echo "<table border='1' cellpadding='2' cellspacing='0' width='100%'>
                <caption><font color='red' size='10'>Serie D</font></caption>
                <tr>
                  <td>Fecha</td>
                  <td>Lote</td>
                  <td>Nombre</td>
